...
The critical error comes from [RMM](https://docs.nvidia.com/rmm/latest/), we're attempting to allocate a [`DeviceBuffer`](https://docs.nvidia.com/rmm/latest/python/rmm/#rmm.DeviceBuffer) on the scheduler, but there is no GPU available to do so:
